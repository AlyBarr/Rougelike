...
public class AStar : MonoBehaviour {
    ...
    public Vector2 Compute(Vector2Int start, Vector2Int goal) {
        if (MapManager.instance == null) {
            Debug.LogError("MapManager.instance is not initialized!");
            return Vector2.zero;  // Early exit
        }

        if (MapManager.instance.FloorMap == null) {
            Debug.LogError("MapManager.instance.FloorMap is not initialized!");
            return Vector2.zero;  // Early exit
        }
        ...
        // If no path was found or path is empty
        ...
        Vector3 newPosition = transform.position + (Vector3)stepDirection;
        if (GameManager.instance.GetActorAtLocation(new Vector3Int(Mathf.FloorToInt(newPosition.x), Mathf.FloorToInt(newPosition.y), 0))) {
            Debug.LogWarning("Destination is blocked by an actor!");
            return Vector2.zero;  // Avoid invalid move
        }
    ...
    private List<Node> FindNeighbours(Vector2Int parentPosition, Vector2Int start) {
      if (MapManager.instance == null || MapManager.instance.FloorMap == null) {
          Debug.LogError("MapManager or FloorMap is not initialized in FindNeighbours!");
          return new List<Node>();  // Early exit with empty list
      }

      List<Node> neighbours = new List<Node>();

      // Get the tilemap's bounds
      var bounds = MapManager.instance.FloorMap.cellBounds;

      for (int x = -1; x <= 1; x++) {
          for (int y = -1; y <= 1; y++) {
              Vector2Int neighbourPos = new Vector2Int(parentPosition.x - x, parentPosition.y - y);

              // Check if the neighbour is within bounds of the map
              if (neighbourPos.x < bounds.x || neighbourPos.y < bounds.y || 
                  neighbourPos.x >= bounds.x + bounds.size.x || neighbourPos.y >= bounds.y + bounds.size.y) {
                  continue;  // Skip out-of-bounds tiles
              }

              if (y != 0 || x != 0) {
                  if (neighbourPos != start && MapManager.instance.FloorMap.GetTile((Vector3Int)neighbourPos) != null) {
    ...
    private int DetermineGScore(Vector2Int neighbour, Vector2Int current) {
        ...
        if (actor) {
            gScore = 10;  // Additional cost for actors occupying a tile
        }
